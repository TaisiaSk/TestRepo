...
public:
    ...
    BSTree(BSTree const& source) : _root(source._root), _size(source._size) {}
    ...
    void breadth_first_search(iterator pos) const;
};
...
template<typename Type>
void BSTree<Type>::breadth_first_search(iterator pos) const
{
    Node* p = pos._curr;
    std::queue<Node*> q;
    if (p == nullptr)
